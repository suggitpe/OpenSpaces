...
import com.ubs.gfit.openspaces.javaprofilers.producersandconsumers.Consumer;
import com.ubs.gfit.openspaces.javaprofilers.producersandconsumers.SlowStartingProducer;
...
public class ProducersAndConsumersTest {
    ...
    private static final Logger LOGGER = LoggerFactory
	    .getLogger(ProducersAndConsumersTest.class);
	...
	ExecutorService producersService = Executors
		.newFixedThreadPool(numberOfProducers);
	ExecutorService consumersService = Executors
		.newFixedThreadPool(numberOfConsumers);
    ...
    private void waitForConsumersToFinish(List<Future<Long>> consumersFutures) throws InterruptedException, ExecutionException {
	for (Future<Long> future : consumersFutures) {
	    LOGGER.info("Consumer finished after processing {} messages", future.get());
	}
    }

    private void waitForProducersToFinish(List<Future<Long>> producersFutures) throws InterruptedException, ExecutionException {
    ...
    private List<Future<Long>> startConsumers(ExecutorService consumersThreadPool,
	    List<Consumer> consumers) {
	List<Future<Long>> consumersFutures = new ArrayList<Future<Long>>();

	for (Consumer consumer : consumers) {
	    Future<Long> consumerFuture = consumersThreadPool
		    .submit(consumer);
    ...
    private List<Future<Long>> startProducers(
	    ExecutorService producersThreadPool) {
	List<Future<Long>> producerFutures = new ArrayList<Future<Long>>();

	for (int i = 0; i < numberOfProducers; i++) {
	    Future<Long> producerFuture = producersThreadPool
		    .submit(applicationContext.getBean(SlowStartingProducer.class));
