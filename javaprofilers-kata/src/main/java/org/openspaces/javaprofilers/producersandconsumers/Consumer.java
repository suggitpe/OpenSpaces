...
public class Consumer implements Callable<Long> {
	...
	private void persistMessage(Message extractedMessage) throws InterruptedException {
		LOGGER.trace(
				"Persisting incoming message from {} to {}, with subject: {}",
				new Object[] { extractedMessage.getSender(), extractedMessage.getRecipient(),
	...
	private void simulatePersistenceOperation(byte[] attachments) throws InterruptedException {
		// Simulate serialization
		for (int i = 0; i < 100 * Math.random(); i++) {
			byte[] attachmentsCopy = new byte[attachments.length];
			System.arraycopy(attachments, 0, attachmentsCopy, 0, attachments.length);
		}
	...
	private Message pollForMessage() throws InterruptedException {
		return messageQueue.poll(MESSAGE_POLLING_INTERVAL_MILLISECONDS, TimeUnit.MILLISECONDS);
	}
